export async function onRequest(context) {
  ...
  if (error) return new Response('Authorization error: ' + error, { status: 400 });
  if (!code) return new Response('Missing code', { status: 400 });

  const CLIENT_ID = env.SPOTIFY_CLIENT_ID;
  const CLIENT_SECRET = env.SPOTIFY_CLIENT_SECRET;
  const REDIRECT_URI = env.SPOTIFY_REDIRECT_URI || `https://${request.headers.get('host')}/api/auth/callback`;
  ...
  if (!tokenRes.ok) {
    return new Response(JSON.stringify(data), { status: tokenRes.status, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  // store tokens in KV (binding name: SPOTIFY_TOKENS)
  try {
